...
*/
// Adapted from React Bits "Shuffle" (MIT).
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  const scrollTriggerStart = useMemo(() => {
    ...
    let sign = ''
    if (mv < 0) sign = `-=${Math.abs(mv)}${mu}`
    else if (mv > 0) sign = `+=${mv}${mu}`
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, {
                y: (_index, t: HTMLElement) =>
                  Number.parseFloat(t.getAttribute('data-start-y') || '0'),
              })
            } else {
              gsap.set(strips, {
                x: (_index, t: HTMLElement) =>
                  Number.parseFloat(t.getAttribute('data-start-x') || '0'),
              })
            }
            onShuffleComplete?.()
          },
        ...
        const addTween = (targets: HTMLElement[], at: number) => {
          ...
          if (isVertical) {
            vars.y = (_index: number, t: HTMLElement) =>
              Number.parseFloat(t.getAttribute('data-final-y') || '0')
          } else {
            vars.x = (_index: number, t: HTMLElement) =>
              Number.parseFloat(t.getAttribute('data-final-x') || '0')
          }
